...
| Areas (13 raids)    | `theSlimePond`, `divineArcheology`, `ancientGraveDigging`, `imperialRescue`, `theCultistRebels`, `theDreadfulAscent`, `theLostExpedition`, `celestialMothership`, `theDireDescent`, `sleepingPlanet`, `kaunis`, `theTower`, `sanguineCrucible` |
| Items / market      | `items[]`, `knownRecipes`, `marketListings[]`, `soldMarketItems[]`, `merchantRegularStockItems[]`, `merchantSpecialReserve`, `newMerchantRegularItems`, `newMerchantSpecialItems`, `blackMarketStock[]`, `blackMarketActive`, `newBlackMarketItems`, `blackMarketMissedDays`, `completedWorkshopItems[]`, `workshopQueue[]` |
| Pets                | `pets[]`, `t4Pet`                                                   |
...
| Settings            | `settingsLanguage`, `settingConfirmRetreat`, `settingConfirmSwap`, `settingConfirmUpgrade`, `settingCraftMaxAmount`, `settingSellMaxAmount`, `settingVerboseLogs`, `settingAutoOpenDungeonDetail`, `settingColorblindMode`, `settingClaimAllChests`, `shownDialogRaid`, `shownDialogEpicRaid` |
| Mod progression     | `imperialKills`, `idleTimeCapHours`, `lootCap` (0 = unset). Legacy saves are migrated by unpacking the old packed `redeem_m975nfu5` int (bits 0-9 kills / 10-17 idle hours / 18-31 loot cap); the `RESETCAPS` redeem zeroes the two caps and wipes the idle/loot bits of `redeem_m975nfu5` (kill bits kept) so stale overrides cannot resurrect |
